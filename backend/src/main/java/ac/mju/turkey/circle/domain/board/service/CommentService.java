...
public class CommentService {
    ...
    public List<CommentDto.TreeResponse> findCommentsByPost(Long postId) {
        Post foundPost = postRepository.findById(postId).orElseThrow(() -> new RestException(ErrorCode.BOARD_POST_NOT_FOUND));


        return commentQueryRepository.findCommentsWithChildrenByPost(foundPost).stream().map(CommentDto.TreeResponse::from).toList();
    }
    ...
    public CommentDto.Response createComment(CommentDto.Request request, Long postId) {
        ...
        notificationService.sendNotification(NotificationDto.Request.of("게시물에 댓글이 달렸습니다.", saved.getContent(), foundPost.getCreatedBy().getEmail()), LinkBuilder.from(saved));
    ...
    public CommentDto.Response createReply(CommentDto.Request request, Long parentId) {
        Comment parent = commentQueryRepository.findById(parentId).orElseThrow(() -> new RestException(ErrorCode.BOARD_PARENT_COMMENT_NOT_FOUND));
        ...
        notificationService.sendNotification(NotificationDto.Request.of("댓글에 대댓글이 달렸습니다.", saved.getContent(), parent.getCreatedBy().getEmail()), LinkBuilder.from(saved));
    ...
    public CommentDto.Response updateComment(Long id, CommentDto.Request request) {
        Comment toUpdate = commentQueryRepository.findById(id).orElseThrow(() -> new RestException(ErrorCode.GLOBAL_NOT_FOUND));
    ...
    public void deleteComment(Long commentId) {
        Comment toDelete = commentQueryRepository.findById(commentId).orElseThrow(() -> new RestException(ErrorCode.GLOBAL_NOT_FOUND));
